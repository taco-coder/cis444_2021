...
def handle_request():
    ...
    if 'book_name' in session:
        session['book_name'] = session.get('book_name') + ";" + request.form['name'] 
    else:
        session['book_name'] = request.form['name']
    if 'book_price' in session:
        session['book_price'] = session.get('book_price') + ";" + request.form['price'] 
    else:
